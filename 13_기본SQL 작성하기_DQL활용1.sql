...
SELECT e.employee_id, e.first_name, e.last_name, e.email, e.phone_number
  FROM employees e;  -- employess 테이블의 별명(alias)을 e로 지정한다.
...
SELECT last_name, department_id
  FROM employees
 WHERE employee_id = 150;  -- 성능이 떨어지는 쿼리(자동으로 WHERE TO_NUMBER(employee_id) = 150;) >> PK이기때문에 인덱스가 붙어있어서 함수 적응하면 성능 떨어짐
...
SELECT *
  FROM employees
 WHERE (salary != 2500 AND salary != 3500 AND salary != 7000) -- (salary NOT IN(2500, 3500, 7000))
...
SELECT last_name, hire_date
  FROM employees
 ORDER BY hire_date ASC;  -- ASC 생략 가능 / DESC는 불가능
