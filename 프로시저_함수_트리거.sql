...
CREATE OR REPLACE PROCEDURE proc3(in_employee_id IN NUMBER)  -- 타입 작성 시 크기 지정 안 함
IS 
...
-- 문제 세팅
-- BOOK, CUSTOMER, OREDERS 테이블 (DQL_연습문제.sql 참조)
...
-- 1. ORDERS 테이블에 주문 기록이 삽입된다. (ORDER_NO는 시퀀스 처리, SALES_PRICE는 BOOK테이블의 PRICE의 90%)
-- 2. CUSTOMER 테이블에 주문총액(구매수량 * 판매가격)의 10%를 POINT에 더해 준다.
...
EXEC proc_order(1, 1, 2);  -- 1번 구매자가 1번책을 2권 구매한다.
...
BEGIN
    total := 0;
    FOR i IN 1 .. n LOOP  -- 1 .. n 최소 1, 최대 n
        total := total + i;
    END LOOP;
    RETURN total;  -- 반환
END get_total;
